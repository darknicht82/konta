...
class admin_home extends controller
{
    ...
    {
        parent::__construct(__CLASS__, 'Panel de control', 'Administrador', FALSE, TRUE, FALSE, 'bi bi-gear-wide-connected');
    }
    ...
    protected function private_core()
    {
        ...
        $this->step = $this->var->simple_get('install_step');
    ...
    private function exec_actions()
    {
        ...
        if (filter_input(INPUT_GET, 'skip')) {
            if ($this->step == '1') {
                $this->step = '2';
                $this->var->simple_save('install_step', $this->step);
            }
            return;
        }
    ...
    private function all_pages()
    {
        ...
        foreach ($this->plugin_manager->enabled() as $plugin) {
            if (!file_exists(JG_FOLDER . '/plugins/' . $plugin . '/controller')) {
                continue;
            }

            foreach (file_manager::scan_files(JG_FOLDER . '/plugins/' . $plugin . '/controller', 'php') as $file_name) {
                $p = new page();
        ...
        foreach (file_manager::scan_files(JG_FOLDER . '/controller', 'php') as $file_name) {
            $p = new page();
            $p->name = substr($file_name, 0, -4);
            $p->exists = TRUE;
            $p->show_on_menu = FALSE;
    ...
    private function enable_page($page)
    {
        ...
        if ('base/controller.php' === $class_name) {
