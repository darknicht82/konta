<?php
/**
 * Controlador de Factura -> Ver Factura Cliente.
 * @author Jonathan Guamba <jonathang_28@hotmail.es>
 */
class ver_factura_cliente extends controller
{
    //variables
    public $factura;
    public $allow_delete;
    ...
    {
        parent::__construct(__CLASS__, 'Ver Factura Cliente', 'Ventas', false, false);
    }
    ...
    private function eliminar_cobro()
    {
        ...
        if ($this->factura) {
            ...
            if ($cobro) {
                if ($cobro->idcobro) {
                    $result = array('error' => 'T', 'msj' => 'El Cobro se encuentra asociado a Cobro Masivo. Verifique y vuelva a intentarlo.');
                } else if ($cobro->delete()) {
            ...
            if ($art) {
                if ($art->tipo == 1 && $art->controlar_stock == 1) {
                    ...
                    if ($art->compuesto) {
                        ...
                        $insumos = $ins0->all_by_idarticulocomp($art->idarticulo, $art->idempresa);
                        $spos    = '';
                        foreach ($insumos as $key => $insumo) {
                            $cant   = 0;
                            $st = $stock->get_by_idestab_idart($this->factura->idestablecimiento, $insumo->idarticulo);
                            if ($st) {
                                $cant += ($st->stock / $insumo->cantidad);
                                if ($spos == '') {
                                    $spos = $cant;
                                } else {
                                    if ($cant < $spos) {
                        ...
                        if ($_POST['cantidad'] > $stock) {
                            $result = array('error' => 'T', 'msj' => 'El articulo ' . $art->nombre . " no tiene el stock suficiente. Stock Actual: " . $stock);
                            $paso   = false;
                        }
                    } else {
    ...
    private function agregar_cobro()
    {
        ...
        if ($this->factura) {
            if ($_POST['num_doc'] != '') {
                ...
                $numDoc = $this->trans_cobros->getNumDocCliente($this->empresa->idempresa, $this->factura->idcliente, $_POST['idformapago'], $_POST['num_doc']);
                if ($numDoc) {
        ...
        if ($this->empresa->activafacelec) {
            if ($this->factura) {
                if ($this->factura->anulado) {
                    ...
                    return false;
                } else if ($this->factura->saldoinicial) {
                ...
                if ($this->factura->getlineas()) {
                    if ($this->factura->estado_sri != 'AUTORIZADO') {
                        ...
                        if ($this->factura->coddocumento == '01') {
                            $carpeta = 'facturas';
                        } else if ($this->factura->coddocumento == '04') {
                            $carpeta = 'notasdecredito';
                        } else if ($this->factura->coddocumento == '05') {
                        ...
                        if (!file_exists($archivoFirmado)) {
                            $result = $autorizar_sri->procesar_documento_sri($this->factura, $this->empresa);
                            if ($result['error'] == 'F') {
                                if (file_exists($archivoFirmado)) {
                                    $archivoXml = simplexml_load_file($archivoFirmado);
                                    if ($archivoXml) {
                                        $this->factura->tipoemision      = 'E';
    ...
    public function validar_edicion()
    {
        ...
        if ($this->factura) {
            if (!$this->allow_modify) {
                ...
                $result = array('error' => 'T', 'msj' => 'La Factura se encuentra Autorizada, no se puede editar.');
            } else {
                $cobros = $this->trans_cobros->all_by_idfacturacli($this->factura->idfacturacli);
                if (count($cobros) > 1) {
                    $result = array('error' => 'T', 'msj' => 'La Factura tiene cobros asociados, no se puede editar.');
                } else {
    ...
    private function editar_documento()
    {
        if ($this->factura) {
            if (!$this->allow_modify) {
                ...
                $this->new_advice("La Factura se encuentra Autorizada, no se puede editar.");
                return;
            } else {
                $cobros = $this->trans_cobros->all_by_idfacturacli($this->factura->idfacturacli);
                if (count($cobros) > 1) {
                    $this->new_advice("La Factura tiene cobros asociados, no se puede editar.");
                    return;
                }
            }
            ...
            $this->factura->idfpsri = $_POST['idfpsri'];
            ...
            if ($this->factura->save()) {
                ...
                $this->new_error_msg("Error al editar la Factura, verifique los datos y vuelva a intentarlo");
            }
        } else {
    ...
    private function reenviar_correo()
    {
        if ($this->factura) {
            ...
            $reenvio = $this->enviar_correo->correo_docs_ventas($this->factura, $this->empresa, false, true, $_POST['copias']);
            if ($reenvio['error'] == 'F') {
