...
/**
 * Controlador de Informes -> Compras.
...
class informes_compras extends controller
{
    ...
    {
        parent::__construct(__CLASS__, 'Compras', 'Informes', 'Proveedores', true, false, 'bi bi-graph-down-arrow');
    }
    ...
    protected function private_core()
    {
        ...
        } else if (isset($_POST['inf_compras'])) {
            switch ($_POST['inf_compras']) {
                case 'totalizado':
                    if ($_POST['formato'] == 'pdf') {
                        $this->pdf_totalizado_compras();
                    } else if ($_POST['formato'] == 'xls') {
                        $this->xls_totalizado_compras();
                    }
                    break;
                case 'desglosado':
                    if ($_POST['formato'] == 'pdf') {
                        $this->pdf_desglosado_compras();
                    } else if ($_POST['formato'] == 'xls') {
                        $this->xls_desglosado_compras();
                    }
                    break;
                default:
                    break;
    ...
    private function init_filter()
    {
        ...
        $this->iddocumento = false;
        if (isset($_POST['iddocumento'])) {
            $this->iddocumento = $_POST['iddocumento'];
        }
    ...
    private function pdf_totalizado_compras()
    {
        $datos = $this->facturas_proveedor->listado_facturas($this->idempresa, $this->fec_desde, $this->fec_hasta, $this->idestablecimiento, $this->idproveedor, $this->iddocumento, true);
        if ($datos) {
            $pdf = new PDF('A4', 'L');
            //$pdf = new PDF();
            $pdf->AliasNbPages();
            ...
            $pdf->MultiCell($ancho, $spacing, 'INFORME TOTALIZADO DE COMPRAS', '', 'C');
            $filtro = '';
            if ($this->fec_desde) {
            ...
            foreach ($datos as $key => $d) {
                if ($documento != $d['iddocumento']) {
                    ...
                    if ($documento != '') {
                        ...
                        $pdf->SetWidths(array(138, 20, 20, 20, 20, 20, 20, 20));
                    ...
                    //$pdf->SetXY($x, $y1);
                    $pdf->SetFont('Times', 'B', 10);
                    $pdf->SetWidths(array(30, 21, 67, 20, 20, 20, 20, 20, 20, 20, 20));
                    $pdf->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));
                    $pdf->Titulos(array('N. Documento', 'F. Emisión', 'Proveedor', 'Estab.', 'Bas. Grav.', 'Base 0%', 'Ot. Bases', 'Descuento', 'Ot. Imp', 'IVA', 'Total'), $spacing);
                    $basegr = 0;
                    $base0  = 0;
                    $otbase = 0;
                    $dto    = 0;
                ...
                $oti = floatval($d['totalice'] + $d['totalirbp']);
            ...
            $pdf->show('listado_totfacturas_' . $this->user->nick . '.pdf');
        } else {
            $this->new_advice("Sin Datos para generar el reporte.");
        }
    }

    private function xls_totalizado_compras()
    {
        $datos = $this->facturas_proveedor->listado_facturas($this->idempresa, $this->fec_desde, $this->fec_hasta, $this->idestablecimiento, $this->idproveedor, $this->iddocumento, true);
        if ($datos) {
            ...
            $titulo1 = "INFORME TOTALIZADO DE COMPRAS";
            ...
            $objPHPExcel->getActiveSheet()->getStyle('A1:R' . $i)->applyFromArray(styleCabeceraReporte());

            //agrego la cabecera de la tabla
            $i++;
            $objPHPExcel->getActiveSheet()
                ->setCellValue('A' . $i, 'Documento')
                ->setCellValue('B' . $i, 'Num. Documento')
                ->setCellValue('C' . $i, 'Fecha Emisión')
                ->setCellValue('D' . $i, 'identificacion')
                ->setCellValue('E' . $i, 'Proveedor')
                ->setCellValue('F' . $i, 'Observaciones')
                ->setCellValue('G' . $i, 'Establecimiento')
                ->setCellValue('H' . $i, 'Base Gravada')
                ->setCellValue('I' . $i, 'Base 0%')
                ->setCellValue('J' . $i, 'Base No Objeto de IVA')
                ->setCellValue('K' . $i, 'Base Excento de IVA')
                ->setCellValue('L' . $i, 'Subtotal Sin Impuestos')
                ->setCellValue('M' . $i, 'Descuento')
                ->setCellValue('N' . $i, 'ICE')
                ->setCellValue('O' . $i, 'IRBPNR')
                ->setCellValue('P' . $i, 'IVA')
                ->setCellValue('Q' . $i, 'TOTAL')
                ->setCellValue('R' . $i, 'Numero Autorización');
            $objPHPExcel->getActiveSheet()->getStyle('A' . $i . ':R' . $i)->applyFromArray(styleNombreColumnas());
            $objPHPExcel->getActiveSheet()->setAutoFilter('A' . $i . ':R' . $i);
            ...
            foreach ($datos as $key => $d) {
                ...
                $objPHPExcel->getActiveSheet()
                    ...
                    ->setCellValue('B' . $i, $d['numero_documento'])
                    ->setCellValue('C' . $i, $d['fec_emision'])
                    ->setCellValueExplicit('D' . $i, $d['identificacion'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('E' . $i, $d['razonsocial'])
                    ->setCellValue('F' . $i, $d['observaciones'])
                    ->setCellValue('G' . $i, $d['establecimiento'])
                    ->setCellValue('H' . $i, ($d['base_gra'] * $multiplicador))
                    ->setCellValue('I' . $i, ($d['base_0'] * $multiplicador))
                    ->setCellValue('J' . $i, ($d['base_noi'] * $multiplicador))
                    ->setCellValue('K' . $i, ($d['base_exc'] * $multiplicador))
                    ->setCellValue('L' . $i, ($subtotal * $multiplicador))
                    ->setCellValue('M' . $i, ($d['totaldescuento'] * $multiplicador))
                    ->setCellValue('N' . $i, ($d['totalice'] * $multiplicador))
                    ->setCellValue('O' . $i, ($d['totalirbp'] * $multiplicador))
                    ->setCellValue('P' . $i, ($d['totaliva'] * $multiplicador))
                    ->setCellValue('Q' . $i, ($d['total'] * $multiplicador))
                    ->setCellValueExplicit('R' . $i, $d['nro_autorizacion'], PHPExcel_Cell_DataType::TYPE_STRING);
                ...
                $t_irbpnr += floatval($d['totalirbp'] * $multiplicador);
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('H' . $i, $t_basegr)
                ->setCellValue('I' . $i, $t_base0)
                ->setCellValue('J' . $i, $t_bnoi)
                ->setCellValue('K' . $i, $t_bexc)
                ->setCellValue('L' . $i, $t_subtotal)
                ->setCellValue('M' . $i, $t_desc)
                ->setCellValue('N' . $i, $t_ice)
                ->setCellValue('O' . $i, $t_irbpnr)
                ->setCellValue('P' . $i, $t_iva)
                ->setCellValue('Q' . $i, $t_total);
            $objPHPExcel->getActiveSheet()->getStyle('A' . $i . ':R' . $i)->applyFromArray(styleNegrita());
            //Auto ajustar cells
            for ($j = 'A'; $j <= 'R'; $j++) {
                $objPHPExcel->getActiveSheet()->getColumnDimension($j)->setAutoSize(true);
            }

            $nombrearchivo = 'listado_totcompras_' . $this->user->nick . '.xlsx';
    ...
    private function pdf_desglosado_compras()
    {
        $articulos = false;
        $servicios = false;
        ...
        if ($this->tipodet) {
            if ($this->tipodet == 1) {
                $articulos = $this->facturas_proveedor->detalle_facturas($this->idempresa, $this->fec_desde, $this->fec_hasta, $this->idestablecimiento, $this->idproveedor, $this->iddocumento, $this->tipodet, true);
            } else if ($this->tipodet == 2) {
                $servicios = $this->facturas_proveedor->detalle_facturas($this->idempresa, $this->fec_desde, $this->fec_hasta, $this->idestablecimiento, $this->idproveedor, $this->iddocumento, $this->tipodet, true);
            }
        } else {
            $articulos = $this->facturas_proveedor->detalle_facturas($this->idempresa, $this->fec_desde, $this->fec_hasta, $this->idestablecimiento, $this->idproveedor, $this->iddocumento, 1, true);
            $servicios = $this->facturas_proveedor->detalle_facturas($this->idempresa, $this->fec_desde, $this->fec_hasta, $this->idestablecimiento, $this->idproveedor, $this->iddocumento, 2, true);
        }
        ...
        if ($articulos || $servicios) {
            ...
            $pdf->SetXY($x, $y1);
            $pdf->SetFont('Times', 'B', 12);
            $pdf->MultiCell($ancho, $spacing, 'INFORME DETALLADO DE COMPRAS', '', 'C');
            $filtro = '';
            if ($this->fec_desde) {
                $filtro .= 'Desde: ' . date('d-m-Y', strtotime($this->fec_desde));
            }
            ...
            if ($articulos) {
                ...
                $pdf->SetWidths(array(28, 45, 17, 30, 20, 20, 20, 20, 20, 15, 20, 20));
                $pdf->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));
                $pdf->Titulos(array('N. Documento', 'Proveedor', 'F. Emisión', 'Descripción', 'Cantidad', 'Precio Unitario', 'Descuento', 'Subtotal', 'Otros. Imp', '% IVA', 'IVA', 'Total'), $spacing);
                ...
                foreach ($articulos as $key => $art) {
                    ...
                    $pdf->SetFont('Times', '', 8);
                    $pdf->SetWidths(array(28, 45, 17, 30, 20, 20, 20, 20, 20, 15, 20, 20));
                    $pdf->SetAligns(array('R', 'L', 'R', 'L', 'R', 'R', 'R', 'R', 'R', 'R', 'R', 'R'));
                    ...
                    $pdf->Fila(
                        array(
                            ...
                            show_numero(floatval($art['porcentaje'])),
                ...
                $pdf->SetFont('Times', 'B', 10);
                $pdf->SetWidths(array(120, 20, 20, 20, 20, 20, 15, 20, 20));
                ...
                foreach ($servicios as $key => $ser) {
                    ...
                    $pdf->Fila(
                        array(
                            ...
                            show_numero(floatval($ser['porcentaje'])),
            ...
            $pdf->show('listado_detcompras_' . $this->user->nick . '.pdf');
        } else {
            $this->new_advice("Sin Datos para generar el reporte.");
        }
    }

    private function xls_desglosado_compras()
    {
        $articulos = false;
        $servicios = false;
        ...
        if ($articulos || $servicios) {
            ...
            $titulo  = $this->empresa->razonsocial;
            $titulo1 = "INFORME DETALLADO DE COMPRAS";
            $objPHPExcel->setActiveSheetIndex(0);
            ...
            $objPHPExcel->getActiveSheet()->getStyle('A1:Q' . $i)->applyFromArray(styleCabeceraReporte());
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('P' . $i, 'TOTAL')
                ->setCellValue('Q' . $i, 'Numero Autorización');

            $objPHPExcel->getActiveSheet()->getStyle('A' . $i . ':Q' . $i)->applyFromArray(styleNombreColumnas());
            $objPHPExcel->getActiveSheet()->setAutoFilter('A' . $i . ':Q' . $i);
            ...
            if ($articulos) {
                foreach ($articulos as $key => $art) {
                    ...
                    $objPHPExcel->getActiveSheet()
                        ...
                        ->setCellValue('N' . $i, floatval($art['porcentaje']))
                        ->setCellValue('O' . $i, floatval($art['valoriva'] * $multiplicador))
                        ->setCellValue('P' . $i, floatval($tot))
                        ->setCellValueExplicit('Q' . $i, $art['nro_autorizacion'], PHPExcel_Cell_DataType::TYPE_STRING);
            ...
            if ($servicios) {
                foreach ($servicios as $key => $ser) {
                    ...
                    $objPHPExcel->getActiveSheet()
                        ...
                        ->setCellValueExplicit('G' . $i, $ser['codprincipal'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ...
                        ->setCellValue('N' . $i, floatval($ser['porcentaje']))
                        ->setCellValue('O' . $i, floatval($ser['valoriva'] * $multiplicador))
                        ->setCellValue('P' . $i, floatval($tot))
                        ->setCellValueExplicit('Q' . $i, $ser['nro_autorizacion'], PHPExcel_Cell_DataType::TYPE_STRING);
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('O' . $i, $t_iva)
                ->setCellValue('P' . $i, $t_total);

            $objPHPExcel->getActiveSheet()->getStyle('A' . $i . ':Q' . $i)->applyFromArray(styleNegrita());
            ...
            $nombrearchivo = 'listado_detcompras_' . $this->user->nick . '.xlsx';
            $objWriter     = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
            $objWriter->save($nombrearchivo);
            $objPHPExcel->disconnectWorksheets();
            unset($objPHPExcel);
