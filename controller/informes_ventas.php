...
/**
 * Controlador de Informes -> Ventas.
...
class informes_ventas extends controller
{
    ...
    {
        parent::__construct(__CLASS__, 'Ventas', 'Informes', 'Clientes', true, false, 'bi bi-graph-up-arrow');
    }
    ...
    private function pdf_totalizado_ventas()
    {
        ...
        if ($datos) {
            ...
            $pdf->MultiCell($ancho, $spacing, 'INFORME TOTALIZADO DE VENTAS', '', 'C');
            $filtro = '';
            if ($this->fec_desde) {
    ...
    private function xls_totalizado_ventas()
    {
        ...
        if ($datos) {
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('D' . $i, 'identificación')
                ...
                ->setCellValue('Q' . $i, 'Número Autorización')
                ->setCellValue('R' . $i, 'Fecha Autorización');
            ...
            foreach ($datos as $key => $d) {
                ...
                $objPHPExcel->getActiveSheet()
                    ...
                    ->setCellValue('O' . $i, ($d['totaliva'] * $multiplicador))
                    ->setCellValue('P' . $i, ($d['total'] * $multiplicador))
                    ->setCellValueExplicit('Q' . $i, $d['nro_autorizacion'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('R' . $i, $d['fec_autorizacion']);
    ...
    private function pdf_desglosado_ventas()
    {
        ...
        if ($articulos || $servicios) {
            ...
            $pdf->SetXY($x, $y1);
            $pdf->SetFont('Times', 'B', 12);
            $pdf->MultiCell($ancho, $spacing, 'INFORME DETALLADO DE VENTAS', '', 'C');
            $filtro = '';
            if ($this->fec_desde) {
                $filtro .= 'Desde: ' . date('d-m-Y', strtotime($this->fec_desde));
            }
            ...
            $pdf->show('listado_detventas_' . $this->user->nick.'.pdf');
        } else {
            $this->new_advice("Sin Datos para generar el reporte.");
        }
    }
    ...
    private function xls_desglosado_ventas()
    {
        ...
        if ($articulos || $servicios) {
            $this->template = false;
            $objPHPExcel    = new PHPExcel();
            $objPHPExcel->getProperties()->setCreator("GSC Systems")->setTitle("INFORME DETALLADO DE VENTAS");
            $sheet = $objPHPExcel->getActiveSheet();
            $sheet->setTitle("Informe Detallado Ventas");
            $titulo  = $this->empresa->razonsocial;
            $titulo1 = "INFORME DETALLADO DE VENTAS";
            $objPHPExcel->setActiveSheetIndex(0);
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('P' . $i, 'Numero de Autorización')
                ->setCellValue('Q' . $i, 'Fecha de Autorización');
            ...
            if ($articulos) {
                foreach ($articulos as $key => $art) {
                    ...
                    $objPHPExcel->getActiveSheet()
                        ...
                        ->setCellValueExplicit('P' . $i, $art['nro_autorizacion'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValue('Q' . $i, $art['fec_autorizacion']);
            ...
            if ($servicios) {
                foreach ($servicios as $key => $ser) {
                    ...
                    $objPHPExcel->getActiveSheet()
                        ...
                        ->setCellValue('O' . $i, floatval($tot))
                        ->setCellValueExplicit('P' . $i, $ser['nro_autorizacion'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValue('Q' . $i, $ser['fec_autorizacion']);
            ...
            $nombrearchivo = 'listado_detventas_' . $this->user->nick . '.xlsx';
            $objWriter     = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
            $objWriter->save($nombrearchivo);
            $objPHPExcel->disconnectWorksheets();
            unset($objPHPExcel);
