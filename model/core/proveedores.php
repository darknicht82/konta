...
class proveedores extends \model
{
    ...
    public function url()
    {
        if (is_null($this->idproveedor)) {
            return 'index.php?page=lista_proveedores';
        }

        return 'index.php?page=ver_proveedor&id=' . $this->idproveedor;
    }

    public function get($idproveedor)
    {
        $data = $this->db->select("SELECT * FROM " . $this->table_name . " WHERE idproveedor = " . $this->var2str($idproveedor) . ";");
        if ($data) {
            return new \proveedores($data[0]);
        }
    ...
    {
        $cob0 = new \trans_pagos();
        return $cob0->getSaldoProveedor($this->idempresa, $this->idproveedor);
    }
    ...
    public function exists()
    {
        ...
        return $this->db->select("SELECT * FROM " . $this->table_name . " WHERE idproveedor = " . $this->var2str($this->idproveedor) . ";");
    }
    ...
    {
        return $this->db->exec("DELETE FROM " . $this->table_name . " WHERE idproveedor = " . $this->var2str($this->idproveedor) . ";");
    }
    ...
    public function search_proveedores($idempresa = '', $query = '', $tipoid = '', $regimen = '', $offset = 0, $limit = JG_ITEM_LIMIT)
    {
        $list = array();
