<?php
require_once 'base/db_engine.php';
...
class mysql extends db_engine
{
    ...
    public function check_table_aux($table_name)
    {
        ...
        $data = $this->select("SHOW TABLE STATUS FROM `" . JG_DB_NAME . "` LIKE '" . $table_name . "';");
        ...
        foreach ($xml_cols as $xml_col) {
            ...
            if (empty($db_col)) {
                ...
                if ($xml_col['tipo'] == 'serial') {
                    $sql .= '`' . $xml_col['nombre'] . '` ' . JG_DB_INTEGER . ' NOT NULL AUTO_INCREMENT;';
                    continue;
                }
    ...
    public function compare_constraints($table_name, $xml_cons, $db_cons, $delete_only = FALSE)
    {
        ...
        if (!empty($xml_cons) && !$delete_only && JG_FOREIGN_KEYS) {
    ...
    public function connect()
    {
        ...
        if (self::$link) {
            ...
            self::$link = @new mysqli(JG_DB_HOST, JG_DB_USER, JG_DB_PASS, JG_DB_NAME, intval(JG_DB_PORT));
    ...
    public function generate_table($table_name, $xml_cols, $xml_cons)
    {
        ...
        foreach ($xml_cols as $col) {
            ...
            if ($col['tipo'] == 'serial') {
                $sql .= '`' . $col['nombre'] . '` ' . JG_DB_INTEGER . ' NOT NULL AUTO_INCREMENT';
            } else {
    ...
    public function list_tables()
    {
        ...
        if ($aux) {
            foreach ($aux as $a) {
                if (isset($a['Tables_in_' . JG_DB_NAME])) {
                    $tables[] = array('name' => $a['Tables_in_' . JG_DB_NAME]);
                }
            }
        }
     ...
     * @return array
     */
    public function select_limit($sql, $limit = JG_ITEM_LIMIT, $offset = 0)
    {
        ...
        if (!empty($xml_cons)) {
            foreach ($xml_cons as $res) {
                if (strstr(strtolower($res['consulta']), 'primary key')) {
                    $sql .= ', ' . $res['consulta'];
                } else if (JG_FOREIGN_KEYS || substr($res['consulta'], 0, 11) != 'FOREIGN KEY') {
