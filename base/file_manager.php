...
class file_manager
{
    ...
    {
        if (!file_exists(JG_FOLDER . '/.htaccess')) {
            $txt = file_get_contents(JG_FOLDER . '/htaccess-sample');
            file_put_contents(JG_FOLDER . '/.htaccess', $txt);
        }

        /// ahora comprobamos el de tmp/XXXXX/private_keys
        if (file_exists(JG_FOLDER . '/tmp/' . JG_TMP_NAME . 'private_keys') && !file_exists(JG_FOLDER . '/tmp/' . JG_TMP_NAME . 'private_keys/.htaccess')) {
            file_put_contents(JG_FOLDER . '/tmp/' . JG_TMP_NAME . 'private_keys/.htaccess', 'Deny from all');
        }
    }
    ...
    {
        foreach (self::scan_files(JG_FOLDER . '/tmp/' . JG_TMP_NAME, 'php') as $file_name) {
            unlink(JG_FOLDER . '/tmp/' . JG_TMP_NAME . $file_name);
        }
    }
    ...
    {
        $notwritable = [];
        foreach (static::scan_folder(JG_FOLDER, true) as $folder) {
