...
class divisa_tools
{
    ...
    public function show_precio($precio = 0, $coddivisa = FALSE, $simbolo = TRUE, $dec = JG_NF0)
    {
        ...
        if (JG_POS_DIVISA == 'right') {
            if ($simbolo) {
                return number_format($precio, $dec, JG_NF1, JG_NF2) . ' ' . $this->simbolo_divisa($coddivisa);
            }

            return number_format($precio, $dec, JG_NF1, JG_NF2) . ' ' . $coddivisa;
        }

        if ($simbolo) {
            return $this->simbolo_divisa($coddivisa) . number_format($precio, $dec, JG_NF1, JG_NF2);
        }

        return $coddivisa . ' ' . number_format($precio, $dec, JG_NF1, JG_NF2);
    }
    ...
    public function show_numero($num = 0, $decimales = JG_NF0, $js = FALSE)
    {
        ...
        return number_format($num, $decimales, JG_NF1, JG_NF2);
    }
