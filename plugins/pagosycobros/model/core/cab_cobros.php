...
class cab_cobros extends \model
{
    public $nombre_fp;
    public function __construct($data = false)
    {
        parent::__construct('cab_cobros');
        if ($data) {
    ...
    public function get($idcobro)
    {
        $data = $this->db->select("SELECT * FROM " . $this->table_name . " WHERE idcobro = " . $this->var2str($idcobro) . ";");
        if ($data) {
            return new \cab_cobros($data[0]);
        }
    ...
    public function getDetalle()
    {
        ...
        return $lineas->all_by_idcobro($this->idcobro);
    }
    ...
    {
        if (is_null($this->idcobro)) {
            return false;
        }

        return $this->db->select("SELECT * FROM " . $this->table_name . " WHERE idcobro = " . $this->var2str($this->idcobro) . ";");
    }
    ...
    public function save()
    {
        if ($this->test()) {
            ...
            if ($this->exists()) {
                ...
                . "  WHERE idcobro = " . $this->var2str($this->idcobro) . ";";
            } else {
                $insert = true;
    ...
    public function delete()
    {
        if ($this->beforeDelete()) {
            if ($this->db->exec("DELETE FROM " . $this->table_name . " WHERE idcobro = " . $this->var2str($this->idcobro) . ";")) {
                //si se elimina borro los cobros asociados.
                $trans_cobros = new \trans_cobros();
                if (!$trans_cobros->deletebyIdCobro($this->idcobro)) {
                    $this->new_error_msg('Error al eliminar cobros de las Facturas.');
                    return false;
                }
            } else {
                return false;
    ...
    {
        if ($this->anulado) {
            $this->new_error_msg('El cobro se encuentra anulado, no es posible eliminar.');
            return false;
        }
    ...
    public function buscarCobros($idempresa = '', $query = '', $idcliente = '', $idformapago = '', $fechadesde = '', $fechahasta = '', $sanuladas = false, $anuladas = false, $offset = 0, $limit = JG_ITEM_LIMIT)
    {
        $list = array();
