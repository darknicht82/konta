...
class cab_devolucion_proveedor extends \model
{
    public $nombre_fp;
    public function __construct($data = false)
    {
        parent::__construct('cab_devolucion_proveedor');
        if ($data) {
    ...
    public function get($iddevolucionprov)
    {
        $data = $this->db->select("SELECT * FROM " . $this->table_name . " WHERE iddevolucionprov = " . $this->var2str($iddevolucionprov) . ";");
        if ($data) {
            return new \cab_devolucion_proveedor($data[0]);
        }
    ...
    public function getDetalle()
    {
        ...
        return $lineas->all_by_iddevolucionprov($this->iddevolucionprov);
    }
    ...
    {
        if (is_null($this->iddevolucionprov)) {
            return false;
        }

        return $this->db->select("SELECT * FROM " . $this->table_name . " WHERE iddevolucionprov = " . $this->var2str($this->iddevolucionprov) . ";");
    }
    ...
    public function save()
    {
        if ($this->test()) {
            ...
            if ($this->exists()) {
                ...
                . "  WHERE iddevolucionprov = " . $this->var2str($this->iddevolucionprov) . ";";
            } else {
                $insert = true;
            ...
            if ($this->beforeSave()) {
                if ($this->db->exec($sql)) {
                    if ($insert) {
                        $this->iddevolucionprov = $this->db->lastval();
                    }
                    return true;
                }
            }
    ...
    public function delete()
    {
        if ($this->beforeDelete()) {
            if ($this->db->exec("DELETE FROM " . $this->table_name . " WHERE iddevolucionprov = " . $this->var2str($this->iddevolucionprov) . ";")) {
                //si se elimina borro los pagos asociados.
                $trans_pagos = new \trans_pagos();
                if (!$trans_pagos->deletebyIdDevolucion($this->iddevolucionprov)) {
                    $this->new_error_msg('Error al eliminar pagos de la Devolucion.');
                    return false;
                }
            } else {
                return false;
    ...
    public function all()
    {
        ...
        if ($data) {
            foreach ($data as $p) {
                $list[] = new \cab_devolucion_proveedor($p);
            }
        }
    ...
    public function buscarDevolucionProv($idempresa = '', $query = '', $idproveedor = '', $idformapago = '', $fechadesde = '', $fechahasta = '', $sproveedor = false, $anuladas = false, $offset = 0, $limit = JG_ITEM_LIMIT)
    {
        $list = array();
