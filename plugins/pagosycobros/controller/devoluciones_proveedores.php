<?php
/**
 * Controlador de Compras -> Anticipos -> Devoluciones
 * @author Jonathan Guamba <jonathang_28@hotmail.es>
 */
class devoluciones_proveedores extends controller
{
    //Filtros
    public $query;
    public $tipo;
    ...
    {
        parent::__construct(__CLASS__, 'Devoluciones', 'Compras', 'Anticipos', true, false, 'bi bi-cash-coin');
    }
    ...
    protected function private_core()
    {
        ...
        $this->access_crear = $this->user->have_access_to('crear_devolucion_proveedor');
    ...
    private function init_filter()
    {
        ...
        $this->sproveedor = isset($_REQUEST['sproveedor']);
        if (isset($_REQUEST['sproveedor'])) {
            $this->filtros .= '&sproveedor';
        }
    }
    ...
    {
        if ($offset == 0) {
            $this->resultados = $this->cab_devolucion_proveedor->buscarDevolucionProv($this->idempresa, $this->query, $this->idproveedor, $this->idformapago, $this->desde, $this->hasta, $this->sproveedor, $this->anulados, $this->offset);
        } else {
            $this->cantidad = $this->cab_devolucion_proveedor->buscarDevolucionProv($this->idempresa, $this->query, $this->idproveedor, $this->idformapago, $this->desde, $this->hasta, $this->sproveedor, $this->anulados, -1);
            if (!$this->cantidad) {
                $this->cantidad = 0;
            }
        }
    ...
    {
        $pago = $this->cab_devolucion_proveedor->get($_GET['delete']);
        if ($pago) {
