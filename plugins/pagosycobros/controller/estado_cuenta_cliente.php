...
/**
 * Controlador de Clientes -> Estados de Cuenta.
 * @author Jonathan Guamba <jonathang_28@hotmail.es>
 */
class estado_cuenta_cliente extends controller
{
    public $idempresa;
    public $fec_desde;
    public $fec_hasta;
    ...
    {
        parent::__construct(__CLASS__, 'Estados de Cuenta', 'Informes', 'Clientes', true, false, 'bi bi-bag-plus-fill');
    }
    ...
    {
        $datos = $this->trans_cobros->getEstadoCuentaCliente($this->idempresa, $this->idcliente, $this->fec_desde, $this->fec_hasta, $this->idestablecimiento, $this->estado);
        if ($datos) {
            $pdf = new PDF();
            //$pdf = new PDF();
            $pdf->AliasNbPages();
            ...
            $pdf->MultiCell($ancho, $spacing, 'ESTADO DE CUENTA TOTALIZADO DE CLIENTES', '', 'C');
            $filtro = '';
            if ($this->fec_desde) {
            ...
            $pdf->show('estado_cuenta_cliente_' . $this->user->nick . '.pdf');
        } else {
    ...
    private function xls_totalizado_est_cuenta()
    {
        ...
        if ($datos) {
            ...
            $objPHPExcel->getProperties()->setCreator("GSC Systems")->setTitle("ESTADO DE CUENTA TOTALIZADO DE CLIENTES");
            $sheet = $objPHPExcel->getActiveSheet();
            $sheet->setTitle("Estado de Cuenta");
            $titulo  = $this->empresa->razonsocial;
            $titulo1 = "ESTADO DE CUENTA TOTALIZADO DE CLIENTES";
            ...
            $nombrearchivo = 'estado_cuenta_cliente_' . $this->user->nick . '.xlsx';
