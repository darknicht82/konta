$(document).ready(function() {
    ...
    $("#f_nuevo_anticipo").submit(function(event) {
        ...
        $.ajax({
            ...
            error: function() {
                alert("Error al intentar guardar el Anticipo del Proveedor!");
            }
        });
                ...
                if (datos.error == 'T') {
                    ...
                    if (allow_modify == 1) {
                        $("#idproveedor").select2({
                            ...
                            data: [{
                                id: datos.proveedor.idproveedor,
                                text: datos.proveedor.identificacion + " - " + datos.proveedor.razonsocial
                            }],
...
function eliminar_anticipo(idanticipo, numero) {
    bootbox.confirm({
        message: '¿Realmente desea eliminar el Anticipo ' + numero + '?',
        title: '<b>Atención</b>',
        callback: function(result) {
            if (result) {
                window.location.href = 'index.php?page=lista_anticipos_proveedores&delete=' + idanticipo;
            }
        }
    });
}
