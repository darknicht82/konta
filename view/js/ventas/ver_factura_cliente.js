$(document).ready(function() {
   ...
   $("#f_nuevo_cobro").submit(function (event) {
      $("#btn_add_cobro").attr('disabled', true);
      event.preventDefault();
      $.ajax({
         async: false,
         url: factura_url,
      ...
      $("#btn_add_cobro").attr('disabled', false);
   });
   ...
   $("#b_eliminar_factura").click(function(event) {
      ...
      bootbox.confirm({
         ...
         callback: function(result) {
            if (result) {
               window.location.href = 'index.php?page=lista_facturas_cliente&delete='+idfacturacli;
            }
         }
      });
   });
   ...
   $("#b_anular_documento").click(function(event) {
      event.preventDefault();
      bootbox.confirm({
         message: '¿Realmente desea anular el Documento?',
         title: '<b>Atención</b>',
         callback: function(result) {
            if (result) {
               window.location.href = 'index.php?page=lista_facturas_cliente&anular='+idfacturacli;
            }
         }
      });
   });
   ...
   $.ajax({
      ...
      success: function(data){
         ...
         if (datos.error == 'T') {
            ...
            $("#idfpsri").val(datos.factura.idfpsri);
            //Datos del cliente
...
function listar_cobros()
{
   $.ajax({
      ...
      error: function(){
          alert("Error al intentar buscar los cobros de la factura!");
      }
   });
}
...
function eliminar_cobro(idcobro) {
   bootbox.confirm({
      message: '¿Realmente desea eliminar el cobro de la Factura?',
      title: '<b>Atención</b>',
      callback: function(result) {
         if (result) {
            $.ajax({
