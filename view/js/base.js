...
function ver_password(btn, input, icon)
{
    let cambio = document.getElementById(input);
...
$(document).ready(function () {
    ...
    //Convertidos a minusculas
    $(".minusculas").on("keypress", function () {
        let input = $(this);
        setTimeout(function () {
    ...
    //Convertidos a minusculas
    $(".ident").on("keypress", function (event) {
        if(event.shiftKey){
            event.preventDefault();
        }
        let tipoid = $("#tipoid").val();
        if (tipoid == 'R' || tipoid == 'C') {
...
function cambio_ident()
{
    ...
    if (tipoid == 'R') {
        ...
        $("#identificacion").attr('minlength', 8);
